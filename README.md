...
編輯器會內建文字編輯模組、「多媒體處理描述語言」轉譯直譯器、Git版本控制、圖形化剪輯、多媒體資料庫，詳細內容可以從「[多媒體資料庫](https://githib.com/)」(建置好但還沒編輯)了解。

#### 多媒體處理描述語言伺服器
「多媒體處理描述語言伺服器」是一個可以在網頁上面將「多媒體處理描述語言」的描述呈現出媒體的最終樣態，有點是多媒體處理描述語言編輯器，支援
